...
function (exports, shader, framebuffer, data) {
 	...
 	function assembleIntersectionForScanline(vertices, polygon, color, textureCoord, polygonTextureCoord, texture) {
   		...
   		for (var i = polygon.length - 1; i > 0; i = i - 1) {
	 		...
	 		if (i == polygon.length - 1) {
	   			...
	   			lastDerivative = calcDerivative(start,Math.floor(vertices[polygon[i + 1]][1]));
	 		}
	 		if (lastDerivative != 0) {
	   			break;
	 		}
   		...
   		for (var i = 0; i < polygon.length; i++) {
			...
			nextZ = 0.0;
	 		...
	 		// TODO in exercise texture

	 		// Calculate current and save last derivative.
	 		derivative = calcDerivative(currY, nextY);
	 		//console.log("(" + currX + ", " + currY + ") -> ("+ nextX + ", " + nextY + ") - derivative:" + derivative + " lastDerivative " + lastDerivative);
 	...
 	* @parameter normal:
 	...
 	function calcPlaneEquation(vertices, polygon, normal) {
   		...
   		var pp = vertices[polygon[0]];
   		D = -(pp[0] * A + pp[1] * B + pp[2] * C);
		AdivC = -A / C;
   		inverseC = 1 / C;
